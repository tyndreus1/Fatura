...
export default function InvoicePreview({ invoice, bankData }: Props) {
  ...
  return (
    <div className="mx-auto w-full max-w-[794px] rounded-2xl border border-gray-200 bg-white shadow-xl overflow-hidden print:shadow-none print:border-0 print:rounded-none flex flex-col min-h-[1123px]">

      {/* ── Header: logo 120mm flush 0,0 — title 2cm from top ── */}
        ...
        <div className="shrink-0">
          {!logoError && (
            <img
              ...
              alt="Logo"
              className="block w-[453px] h-auto"
      ...
      <div className="flex items-start justify-between px-8 py-4">
        <div className="flex-1">
          ...
          {invoice.customer_address && (
            <p className="mt-0.5 text-xs text-gray-600 whitespace-pre-line max-w-[157px]">{invoice.customer_address}</p>
          )}
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="bg-[#1E0A00] text-white text-xs font-bold tracking-wide">
              <th className="py-1.5 px-3 text-left w-12 align-middle">QTY</th>
              <th className="py-1.5 px-3 text-left align-middle">DESCRIPTION</th>
              <th className="py-1.5 px-3 text-right w-36 align-middle">UNIT PRICE</th>
              <th className="py-1.5 px-3 text-right w-32 align-middle">AMOUNT</th>
            </tr>
          </thead>
          <tbody>
      ...
      <div className="px-8 pb-4 pt-4">
        <div className="ml-auto max-w-[240px] space-y-1.5">
          ...
          {invoice.shipment_cost > 0 && (
            <div className="flex justify-between text-sm text-gray-600"><span>Shipment</span><span>{S} {formatNumber(invoice.shipment_cost)}</span></div>
          )}
          {invoice.discount > 0 && (
            <div className="flex justify-between text-sm text-gray-600"><span>Discount</span><span>-{S} {formatNumber(invoice.discount)}</span></div>
          )}
          {invoice.advance_payment > 0 && (
            <div className="flex justify-between text-sm text-gray-600"><span>Advance</span><span>-{S} {formatNumber(invoice.advance_payment)}</span></div>
          )}
